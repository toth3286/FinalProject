class Test5 extends Thread {
  ...
  private boolean test3( ) {
    ...
    SysLib.cout( "successfully completed with size: " + size + "\n" );
    return true;
  }
  ...
  private boolean test4( ) {
    ...
    SysLib.cerr("Size after close is: " + size + " and fd is: " + fd + "\n");
    size = SysLib.write( fd, buf16 );
    SysLib.cerr("Size after write is: " + size + "\n");
    if ( size > 0 ) {
      SysLib.cout( "writable even after closing the file: size " + size + "\n" );
      return false;
    }
  ...
  private boolean test8( ) {
    ...
    for ( byte i = 0; i < 16; i++ )
      if ( tmpBuf[i] != buf16[i] ) {
        SysLib.cout( "tmpBuf[" + i + "]=" + tmpBuf[i] + " (wrong"+ buf16[i]+")\n" );
        SysLib.close( fd );
        return false;
      }
